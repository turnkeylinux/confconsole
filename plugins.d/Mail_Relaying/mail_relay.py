...
FORMNOTE = (
    ...
    "Note: The relay authentication procedure requires the user password to be"
    " stored in plain text at /etc/postfix/sasl_passwd (readable only by"
    " root). If this is not what you want, you should cancel this"
    " configuration step."
)
...
def run():
    ...
    if choice:
        if choice == "Deconfigure":
            proc = subprocess.run(
                [cmd, "deconfigure"], capture_output=True, text=True
            )
